...
methods.config.seurat <- list(nfeatures=2000,pc_dims=10,resolution=0.01) ###only specify cluster number via resolution
methods.config.tscan <- list(cvcutoff=0.01,k=55:70,minexpr_value=1,minexpr_percent = 0.5)###cluster number specified via k range
...
methods.config.liger <- list(suggestK=F,k.suggest=20,lambda=NULL,resolution=0.01,thresh=NULL)###only specify cluster number via resolution
...
methods.config.pcaReduce <- list(K=1,q=25)
