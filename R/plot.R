...
plot.plot <- function(experiment,results,raw_results){
  ...
  switch(experiment,
    ...
    celltype_number = plot.celltype_number(results,raw_results,dataset),
    celltype_detection = plot.celltype_detection(results,raw_results,dataset),
...
plot.celltype_detection <- function(results,raw_results,dataset){
  fig_path <- str_glue("{result_home}{experiment}/{dataset}/")
  if(!dir.exists(fig_path)){
    dir.create(fig_path,recursive=T)
  } 
  
  celltype_pctg <- experiments.parameters[[experiment]]$type_pctg
  
  results <- utils.get_methods(results)
  all_results <- dplyr::select(dplyr::filter(results,is.na(assigned)),c(ARI,AMI,FMI,methods,unknown_type,supervised)) %>%
    gather("metric","value",-c(methods,unknown_type,supervised))
  
  plot_params <- list(x="methods",y="value",fill="supervised",label="label",xlabel="methods",
                      facet_wrap=T,dodged=T,facet_var='unknown_type',width=10,height=7,nrow=3)
  all_results$label <- round(all_results$value,2)
  plot_params$ylabel <- "ARI"
  plot.bar_plot(all_results[all_results$metric=="ARI",],plot_params,fig_path,"all_ARI.pdf")
  plot_params$ylabel <- "AMI"
  ...
  results_assigned <- dplyr::select(dplyr::filter(results,assigned),c(ARI,AMI,FMI,methods,unknown_type,supervised)) %>%
    gather("metric","value",-c(methods,unknown_type,supervised))
  results_assigned$label <- round(results_assigned$value,2)
  plot_params$ylabel <- "ARI"
  plot.bar_plot(results_assigned[results_assigned$metric=="ARI",],plot_params,fig_path,"assigned_ARI.pdf")
  ...
  results_unassigned <- dplyr::select(dplyr::filter(results,!assigned),c(ARI,AMI,FMI,methods,unknown_type,supervised)) %>%
    gather("metric","value",-c(methods,unknown_type,supervised))
  results_unassigned$label <- round(results_unassigned$value,2)
  plot_params$ylabel <- "ARI"
  plot.bar_plot(results_unassigned[results_unassigned$metric=="ARI",],plot_params,fig_path,"unassigned_ARI.pdf")
  ...
  plot_params <- list(x="unknown_type",y="unlabeled_pctg",group="methods",line_color="methods",point_color="methods",
                      facet_wrap=F,width=2*length(unique(results_unlabeled_pctg$methods)),
                      height=2*length(unique(results_unlabeled_pctg$methods)))
  plot.line_plot(results_unlabeled_pctg,plot_params,fig_path,"unlabeled_pctg.pdf")
  
  ###sankey_plot
  celltype <- "unknown_type"
  celltype1 <- unique(results$unknown_type)
  for(i in celltype1){
    raw_results1 <- raw_results[raw_results$unknown_type==i,]
    ...
    plot.sankey_plot(raw_results1,sankey_plot_params,fig_path,str_glue("sankey_{celltype}_{i}.pdf"))
  }
}
...
plot.celltype_number <- function(results,raw_results,dataset){
  fig_path <- str_glue("{result_home}{experiment}/{dataset}/")
  if(!dir.exists(fig_path)){
    dir.create(fig_path,recursive=T)
  }
  ...
  for(i in celltype_num1){
    ...
    raw_results1 <- gather(raw_results1,"methods","pred",-c(label,type_num,type_pctg,train_dataset,test_dataset)) %>%
      group_by(methods,label,pred) %>%
      summarize(freq=n()) %>%
      filter(freq>0)
...
plot.sankey_plot <- function(raw_results,plot_params,fig_path,fig_name){
  ...
  if(experiment %in% c("celltype_number")){
    ggsave(
      fig_name,
      plot = last_plot(),
      device = 'pdf',
