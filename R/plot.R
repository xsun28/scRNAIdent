...
plot.celltype_number <- function(results,raw_results,fig_path,exp_config){
  ...
  ###sankey_plot
  uniq_sample_plan <- unique(raw_results[["type_num"]])
  for(plan in uniq_sample_plan){
    raw_results1 <- raw_results[raw_results[["type_num"]]==plan,]
    ...
    if("train_dataset" %in% colnames(raw_results1)){
      ...
      raw_results1 <- gather(raw_results1,"methods","pred",-c(label,type_num,dataset))
    } 
    raw_results1 <- dplyr::group_by(raw_results1,methods,label,pred) %>%
      summarize(freq=n()) %>%
      filter(freq>0)
    ...
    plot.sankey_plot(raw_results1,sankey_plot_params,fig_path,str_glue("sankey_type_num_{plan}.pdf"))
  }
}
...
plot.line_plot <- function(results,params,fig_path,fig_name){
  ...
  }else if(!is_null(params$facet_grid)&&params$facet_grid){
    plot_str <- str_glue("{plot_str} + 
